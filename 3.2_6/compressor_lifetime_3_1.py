...
文件名称 (Filename)  : compressor_lifetime_3_1.py
版本号   (Version)   : Rev 1.0
作者     (Author)    : [Zhang zhelei / Michelin]
所属机构 (Company)   : [Fresenius Medical Care / VTC]
修改日期 (Date)      : 2026-04-07
版权声明 (Copyright) : Copyright (C) 2026 [Fresenius Medical Care]. All Rights Reserved.

功能描述 (Description):
    程序用于控制 NI USB-6362/6363 数据采集卡，对压缩机进行自动化寿命测试。

    [Rev 1.0 修改内容]:
    1. 组路由重构为 6 组固定接线模型:
       Group1~6 -> port0/line0:7, port0/line8:15, port0/line16:23,
                   port0/line24:31, port1/line0:7, port1/line8:15
       压力采集固定映射 Group1~6 -> ai0~ai5。
    2. IO 位序统一为:
       0:V1, 1:V2, 2:V3, 3:蜂鸣器, 4:压力计供电, 5:计数供电, 6:计数信号, 7:压缩机。
    3. TestWorker / ManualControlDialog 改为按 group_cfg 直连 DO/AI，
       不再使用 offset 推导。
    4. 前端台架组选择扩展为最多 6 组，连接检测按真实 port/line/ai 执行。
    5. 启动时提示“新版6组固定接线与新IO位序”，日志包含 Group 路由信息。
...
class TestWorker(QThread):
    ...
    def _simulate_response(self, states):
        if states[DO_COMPRESSOR] and states[DO_V1]:
    ...
    def _run_complex_pulse(self):
        for _ in range(5):
            ...
            s1 = [False]*8
            s1[DO_V1] = True
            s1[DO_COMPRESSOR] = True
            s1[DO_V3] = True
            s1[DO_PRESSURE_POWER] = True
            s1[DO_COUNTER_POWER] = True
            self.write_do(s1)
            if not self._sleep_checked(0.1): return
            s2 = [False]*8
            s2[DO_V1] = True
            s2[DO_COMPRESSOR] = True
            s2[DO_PRESSURE_POWER] = True
            s2[DO_COUNTER_POWER] = True
            self.write_do(s2)
            if not self._sleep_checked(0.1): return
        for _ in range(5): 
            if not self.is_running: return
            ...
            s3[DO_V1] = True
            s3[DO_V3] = True
            s3[DO_PRESSURE_POWER] = True
            s3[DO_COUNTER_POWER] = True
            self.write_do(s3)
            if not self._sleep_checked(0.1): return
            s4 = [False]*8
            s4[DO_V1] = True
            s4[DO_PRESSURE_POWER] = True
            s4[DO_COUNTER_POWER] = True
            self.write_do(s4)
            if not self._sleep_checked(0.1): return
...
class ManualControlDialog(QDialog):
    ...
    def update_pressure(self):
        ...
        if SIMULATION_MODE:
            # 仿真逻辑 (保持简单震荡)
            if self.current_states[DO_COMPRESSOR] and self.current_states[DO_V1]:
                self._sim_p += 0.2
