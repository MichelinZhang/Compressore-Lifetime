...
文件名称 (Filename)  : compressor_lifetime_3_2_6.py
版本号   (Version)   : Rev 3.2.6
作者     (Author)    : [Zhang zhelei / Michelin]
所属机构 (Company)   : [Fresenius Medical Care / VTC]
修改日期 (Date)      : 2026-06-25
版权声明 (Copyright) : Copyright (C) 2026 [Fresenius Medical Care]. All Rights Reserved.

功能描述 (Description):
    控制 NI USB-6362/6363 DAQ，对压缩机进行多工位自动化寿命耐久测试。
    打包发布: 在项目根目录执行 scripts\\build_nuitka.ps1 或 build.bat
    ...
    Phase2 x14轮: 每轮 10 脉冲(9x简单 + 1x复杂) + 57s 泄压
      - 简单脉冲: 压缩机 1s ON / 1s OFF
      - 复杂脉冲: 压缩机同上；仅 V3 以 100ms 方波切换；V1/V2 恒低
    计数器触发 1s -> 下一 Cycle
...
IDX_MAP = {
    ...
    DO_V3: "V3 阀门",
    DO_BUZZER: "蜂鸣器/报警",
    DO_PRESSURE_POWER: "压力计供电",
    DO_COUNTER_POWER: "计数供电",
    DO_COUNTER_SIGNAL: "计数信号",
    DO_COMPRESSOR: "压缩机",
}

GROUP_MAP = [
    {"group_id": 1, "port": 0, "line_start": 0, "line_end": 7, "ai_channel": 0},
    {"group_id": 2, "port": 0, "line_start": 8, "line_end": 15, "ai_channel": 1},
...
def get_group_cfg(index):
    idx = int(index)
    if idx < 0 or idx >= len(GROUP_MAP):
        raise ValueError(f"Group index 超出范围: {index}")
    return dict(GROUP_MAP[idx])


def format_group_cfg(group_cfg):
    return (
        f"Group {group_cfg['group_id']} "
        f"(port{group_cfg['port']}/line{group_cfg['line_start']}:{group_cfg['line_end']}, "
        f"ai{group_cfg['ai_channel']})"
    )


def build_do_lines(dev_name, group_cfg):
    return (
...
def build_ai_chan(dev_name, group_cfg):
...
PHASE_1_REPEAT = 4
PHASE_2_REPEAT = 14
PULSE_HOLD_S = 1.0           # 简单/复杂脉冲 ON、OFF 各段时长 (s)
PULSE_SETTLE_S = 0.01        # OFF->下一状态 切换间隔 (s)，MOSFET 驱动可设 0~0.01
COMPLEX_V3_PERIOD_S = 0.1    # 复杂脉冲 V3 方波周期半宽 (s)
RELEASE_V23_S = 20.0         # 57s 泄压: V2+V3 段
RELEASE_V1_S = 37.0          # 57s 泄压: 静置段 (仅供电)
ENABLE_ENTRY_ANIMATION = False
MAX_STATIONS = 6
...
class TestWorker(QThread):
    ...
    def __init__(self, config, group_cfg, log_dir):
        ...
        try:
            ...
            self.sim_mode = bool(config['simulation'])
            self.station_idx = int(config.get('station_idx', 0))
        ...
        if self.target_cycles <= 0:
            raise ValueError("目标循环次数必须大于 0")
        if self.floor_p < 0:
            raise ValueError("低压阈值不能小于 0")
        if self.target_p <= self.floor_p:
            raise ValueError("高压阈值必须大于低压阈值")
        if self.max_p <= self.target_p:
            raise ValueError("保护上限必须大于高压阈值")
    ...
    def run(self):
        try:
            ...
            self.sig_log.emit(f"启动: {self.dev_name} [{self.group_label}]")
    ...
    def trigger_fault(self, error_msg):
        if self.fault_triggered and self.is_paused:
            return
    ...
    def run_phase_1(self, cycle):
        success_count = 0 
        for i in range(PHASE_1_REPEAT):
            self.check_pause_state() 
            self.step_max_p = 0.0; self.step_min_p = 99.9
            if not self.is_running: return False
            self.sig_status.emit(f"P1 ({i+1}/{PHASE_1_REPEAT}): 初始加压", STATUS_STYLES["run"])
            current_loop_reached = False; in_release_mode = False
            phase_duration = 90.0
            ...
            while True:
                ...
                elapsed = time.monotonic() - t_start - paused_total
                ...
                states[DO_COUNTER_POWER] = True
                if not in_release_mode:
                    states[DO_COMPRESSOR] = True
                    if p >= self.target_p:
                        current_loop_reached = True; in_release_mode = True
                        states[DO_COMPRESSOR] = True
                        states[DO_V2] = True
                        states[DO_V3] = True
                        self.sig_status.emit(f"P1 ({i+1}/{PHASE_1_REPEAT}): 达标泄压", STATUS_STYLES["run"])
                else:
                    states[DO_COMPRESSOR] = True
                    states[DO_V2] = True
                    states[DO_V3] = True
                    if p <= self.floor_p:
                        ...
                        self.sig_status.emit(f"P1 ({i+1}/{PHASE_1_REPEAT}): 重新打压", STATUS_STYLES["run"])
            ...
            if not self.run_release_57s(cycle, f"P1 ({i+1}/{PHASE_1_REPEAT})"): return False
            self.log_csv(cycle, "Phase_1", f"{i+1}/{PHASE_1_REPEAT} Done", self.read_pressure(silent=True))
        if success_count == 0:
    ...
    def run_phase_2(self, cycle):
        for i in range(PHASE_2_REPEAT):
            ...
            for j in range(10):
                ...
                self.sig_status.emit(f"P2 ({i+1}/{PHASE_2_REPEAT}): 脉冲 {j+1}/10", STATUS_STYLES["run"])
                if j == 9:
                    if not self._run_complex_pulse(): return False
                elif not self._run_simple_pulse():
                    return False
            if not self.run_release_57s(cycle, f"P2 ({i+1}/{PHASE_2_REPEAT})"): return False
            self.log_csv(cycle, "Phase_2", f"{i+1}/{PHASE_2_REPEAT} Done", self.read_pressure(silent=True))
        return True
    ...
    def run_release_57s(self, cycle, phase_name):
        ...
        s_a[DO_V2] = True
        s_a[DO_V3] = True
        s_a[DO_PRESSURE_POWER] = True
        s_a[DO_COUNTER_POWER] = True
        self.write_do(s_a)
        if not self.sleep_smart(RELEASE_V23_S): return False
        self.sig_status.emit(f"{phase_name}: 泄压 (V1)", STATUS_STYLES["run"])
        s_b = [False]*8
        s_b[DO_PRESSURE_POWER] = True
        s_b[DO_COUNTER_POWER] = True
        self.write_do(s_b)
        if not self.sleep_smart(RELEASE_V1_S): return False
        return True

    def _wait(self, duration, interval=0.1, *, silent=False, emit_timer=True):
    ...
    def sleep_smart(self, duration, interval=0.1):
        return self._wait(duration, interval=interval, silent=False, emit_timer=True)

    def _sleep_checked(self, duration):
        return self._wait(duration, interval=0.02, silent=True, emit_timer=False)
    ...
    def finalize_success(self):
        ...
        try:
            if self.do_task:
                f = [False]*8
                f[DO_PRESSURE_POWER] = True
                f[DO_COUNTER_POWER] = True
    ...
    def create_log_file(self):
        ...
        station_suffix = f"_St{self.station_idx}" if self.station_idx > 0 else ""
        self.csv_file = os.path.join(
            self.log_dir,
            f"Log_{self.dev_name}_Grp{self.group_cfg['group_id']}"
            f"_P{self.group_cfg['port']}_L{self.group_cfg['line_start']}-{self.group_cfg['line_end']}"
            f"_AI{self.group_cfg['ai_channel']}{station_suffix}_{ts}.csv"
        )
    ...
    def _check_safety(self, val):
        if val > self.max_p and not self.fault_triggered:
    ...
    def _trigger_counter(self):
        ...
        s[DO_V2] = True
        s[DO_V3] = True
        s[DO_PRESSURE_POWER] = True
        s[DO_COUNTER_POWER] = True
        s[DO_COUNTER_SIGNAL] = True
        self.write_do(s)

    def _pulse_hold(self, duration):
        """持稳 DO 状态，读压 10Hz，避免 pulse 期间 DAQ 过载。"""
        start = time.monotonic()
        paused_total = 0.0
        last_read_t = 0.0
        while True:
    ...
    def _run_simple_pulse(self):
        s_on = [False]*8
        s_on[DO_COMPRESSOR] = True
        s_on[DO_PRESSURE_POWER] = True
        s_on[DO_COUNTER_POWER] = True
        self.write_do(s_on, force=True)
        if not self._pulse_hold(PULSE_HOLD_S):
            return False
        s_off = [False]*8
        s_off[DO_PRESSURE_POWER] = True
        s_off[DO_COUNTER_POWER] = True
        self.write_do(s_off, force=True)
        if PULSE_SETTLE_S > 0:
    ...
    def _run_complex_pulse(self):
        if not self._run_complex_pulse_segment(compressor_on=True):
            return False
        if PULSE_SETTLE_S > 0:
            time.sleep(PULSE_SETTLE_S)
        if not self._run_complex_pulse_segment(compressor_on=False):
            return False
        return True

    def _run_complex_pulse_segment(self, compressor_on, duration=PULSE_HOLD_S):
        """复杂脉冲：压缩机 PULSE_HOLD_S 开/关；V3 每 COMPLEX_V3_PERIOD_S 边沿写 DO；V1/V2 恒低。"""
        start = time.monotonic()
        paused_total = 0.0
        last_v3_on = None
        last_read_phase = -1

        def build_states(v3_on):
            states = [False] * 8
            states[DO_PRESSURE_POWER] = True
            states[DO_COUNTER_POWER] = True
            if compressor_on:
...
class ManualControlDialog(QDialog):
    ...
    def init_ui(self):
        ...
        title = QLabel(
            f"IO Control: port{self.group_cfg['port']} "
            f"line{self.group_cfg['line_start']}:{self.group_cfg['line_end']}"
        )
        ...
        if not SIMULATION_MODE and self.do_task:
            try:
                ...
                for i, btn in enumerate(self.buttons):
                    btn.setChecked(i == DO_BUZZER)
                    self.update_btn_style(i, i == DO_BUZZER)
            except Exception as e:
...
class StationWidget(QFrame):
    ...
    def init_ui(self):
        ...
        self.combo_group.addItems([format_group_cfg(cfg) for cfg in GROUP_MAP])
        self.combo_group.setCurrentIndex((self.idx - 1) % len(GROUP_MAP))
    ...
    def _animate_button_feedback(self, btn):
        if not btn.isVisible():
            return
        # Keep feedback lightweight and robust: no opacity/geometry effect on buttons.
        # This avoids occasional invisible start/pause button issues during runtime.
        if btn.graphicsEffect() is not None:
            btn.setGraphicsEffect(None)
        btn.update()

    def _tick_value_labels(self):
        p_diff = self._target_pressure - self._display_pressure
        if abs(p_diff) < 0.01:
        ...
        if self._target_timer is not None:
            ...
            t_diff = self._target_timer - self._display_timer
            if abs(t_diff) < 0.05:
                self._display_timer = self._target_timer
            else:
    ...
    def current_group_label(self):
        return format_group_cfg(self.current_group_cfg())
    ...
    def toggle_connection(self):
        if self.btn_connect.isChecked():
            ...
            if not SIMULATION_MODE:
                try:
                    with nidaqmx.Task() as t_do:
                        t_do.do_channels.add_do_chan(
                            build_do_lines(dev_name, group_cfg),
                            line_grouping=LineGrouping.CHAN_PER_LINE
                        )
                    with nidaqmx.Task() as t_ai:
                        t_ai.ai_channels.add_ai_voltage_chan(
                            build_ai_chan(dev_name, group_cfg),
            ...
            self._animate_button_feedback(self.btn_connect)
            self.btn_start.setEnabled(True) 
            self.global_log.emit(f"[Station {self.idx}] 硬件已连接 {group_label}")
        else:
            group_label = self.current_group_label()
            self.hardware_connected = False
            ...
            self.global_log.emit(f"[Station {self.idx}] 硬件断开 {group_label}")
    ...
    def _build_worker_config(self):
        ...
        try:
            cycles = int(self.in_cycles.text().strip())
        except ValueError:
            raise ValueError("目标循环必须是整数。")

        try:
            target_p = float(self.in_target.text().strip())
            floor_p = float(self.in_floor.text().strip())
            max_p = float(self.in_max.text().strip())
        except ValueError:
            raise ValueError("压力参数必须是数字。")

        if cycles <= 0:
            raise ValueError("目标循环必须大于 0。")
        if floor_p < 0:
            raise ValueError("低压阈值不能小于 0。")
        if target_p <= floor_p:
            raise ValueError("高压阈值必须大于低压阈值。")
    ...
    def start_test(self):
        ...
        self._animate_button_feedback(self.btn_start)
    ...
    def update_start_btn_text(self, mode):
        if mode == "continue":
            self.btn_start.setText("报警消除/继续测试")
            self.btn_start.setStyleSheet("background-color: #34C759; color: white; border: none;")
            self._animate_button_feedback(self.btn_start)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._settings_target_height = 60
        ...
        self.settings_panel.setMaximumHeight(self._settings_target_height)
        ...
        self.lbl_dir = QLabel(self.log_dir)
        ...
        self.settings_anim = QPropertyAnimation(self.settings_panel, b"maximumHeight", self)
        self.settings_anim.setDuration(240)
        self.settings_anim.setEasingCurve(QEasingCurve.Type.OutCubic)
        self.settings_anim.finished.connect(self._on_settings_anim_finished)
    ...
    def add_station(self):
        if len(self.stations) >= MAX_STATIONS:
            ...
            self.append_log(f"系统: 已达到台架上限（{MAX_STATIONS}），忽略新增请求。")
            return
        ...
        self.rearrange_layout(animate=ENABLE_ENTRY_ANIMATION)
    ...
    def _update_add_button_state(self):
        can_add = len(self.stations) < MAX_STATIONS
        self.btn_add.setEnabled(can_add)
        self.btn_add.setToolTip("" if can_add else f"已达上限：最多 {MAX_STATIONS} 个 Station")

    def rearrange_layout(self, animate=False):
        # 先临时移除所有
        ...
        for i, st in enumerate(self.stations):
            ...
            st.setVisible(True)
            if animate:
                self._animate_station_entry(st, i)

    def _animate_station_entry(self, station, order):
        if not ENABLE_ENTRY_ANIMATION:
            return
        # 当前默认关闭卡片入场动画，优先保证布局稳定、避免抖动。
        # 若后续要恢复，建议使用不影响 layout 的轻量效果（例如仅内容渐变）。

    def toggle_settings(self):
        self.settings_anim.stop()
        if self._settings_expanded:
            self._settings_expanded = False
            self.settings_anim.setStartValue(self.settings_panel.maximumHeight())
            self.settings_anim.setEndValue(0)
        else:
            self._settings_expanded = True
            self.settings_panel.setVisible(True)
            self.settings_anim.setStartValue(self.settings_panel.maximumHeight())
            self.settings_anim.setEndValue(self._settings_target_height)
        self.settings_anim.start()
        self.btn_toggle.setText("⚙️ 系统设置 (System Settings) " + ("▼" if self._settings_expanded else "▶"))
    ...
    def toggle_sim(self, s):
        ...
        if any(st.worker and st.worker.isRunning() for st in self.stations):
            QMessageBox.warning(self, "操作受限", "有台架正在运行，不能切换仿真模式。")
            self.chk_sim.blockSignals(True)
            self.chk_sim.setChecked(SIMULATION_MODE)
    ...
    def set_dir(self):
        ...
        if d:
            ...
            self.settings_store.setValue("log_dir", d)
